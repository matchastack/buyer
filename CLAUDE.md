...
- **`wishlist-monitor.ts`** — Wishlist watcher. `watchWishlist` polls `settings.wishlistUrl`, classifies **every** tracked item per poll (matches each wishlist card to a config item by product id from the link href; `cardOutOfStock` overlay ⇒ out_of_stock, else enabled `cardAddToCart` ⇒ in_stock), and `fire`s the item's gate on an out_of_stock→in_stock transition (pure `isRestockTransition`). Mirrors `monitor.ts` challenge-survival; halts loudly on `login_required`. Per-card matching uses `resolveSelectorWithin` (no inline locators). Wishlist selectors are **unverified** — run `verify-selectors` (now covers the wishlist page).
...
- **`auth.ts`** — Session cookies persisted to disk with mode `0o600`. `detectChallenge` checks URL patterns (`/baxia/`, `/block`, `/robot`, `captcha`, `/cdn-cgi/challenge-platform/`, `/awswaf/`, `/sec/`) and DOM markers (captcha frames, slider, rate-limit pages). Any challenge throws `ChallengeDetectedError` — the script never attempts to solve CAPTCHAs, MFA, or rate limits. `isLoggedIn` uses **positive + negative** detection (`login.loggedInIndicator` vs `login.loginLink`) and **fails toward logged-out** when neither matches — do not regress to inferring "logged in" purely from the absence of a login link (that false-positive skipped login on a dead session). `runAuthDebug` (CLI `--auth-debug`) clears cookies and steps through the login visibly, snapshotting each step to `data/debug/auth/`; credential values are never logged.
...
- **`selectors.ts`** — The single source of truth for every DOM selector in the app. `SELECTORS` is structured by page (`login`, `antiBot`, `product`, `cart`, `checkout`, `confirmation`). Each `SelectorSet` has `candidates` (tried in order, first visible wins), `description`, and `required` (true ⇒ throw `SelectorNotFoundError`; false ⇒ return null). Text-based `:has-text()` candidates are listed last as resilient fallbacks. The `wishlist` group (cardList/cardProductLink/cardAddToCart/cardOutOfStock) drives wishlist mode; `resolveSelectorWithin(scope, set)` matches these relative to a single card locator instead of page-wide. **All selectors are unverified against live Lazada pages — run `npm run verify-selectors` (it now also opens the wishlist page) after any Lazada UI change.**
