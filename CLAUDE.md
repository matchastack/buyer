...
- **`decision.ts`** — **Pure functions only.** No I/O, no Playwright, no fs, no console. `shouldProceed`, `isPriceAcceptable`, `computeBackoff`, `computeChallengeBackoff`, `extractProductId` (parses both Lazada URL shapes: `…-i<digits>.html` and id-first `…/i<digits>-s<sku>.html`), `isRestockTransition`, `parseWishlistStock`/`classifyFromWishlistState` (read stock from the wishlist's embedded `lightItemDetailDTO`/`outOfStock` JSON with a string-aware array scanner), `formatOrderSummary`. Keep this discipline — these are the unit-test core. Note: `isPriceAcceptable(null, …)` returns `true` (unknown price is not a skip reason — the user controls payment via the PayNow QR).
